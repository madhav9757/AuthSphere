    ...
    <div className="h-full w-full flex items-center justify-center bg-[#09090b] p-4 font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-[90%] md:max-w-md relative z-10">
        ...
        <form
          ...
          aria-label={t("minimaldark.form_aria", "Sign up form")}
        >
          {/* Name Field */}
          <div className="group space-y-2">
            <label
              htmlFor="name"
              className={`text-[11px] font-bold uppercase tracking-widest ml-1 transition-colors ${errors.name ? "text-red-500" : "text-zinc-500 group-focus-within:text-white"}`}
            >
            ...
            >
              <User
                className={`absolute left-4 top-3.5 h-4 w-4 transition-colors ${errors.name ? "text-red-500" : "text-zinc-500 group-focus-within:text-white"}`}
                aria-hidden="true"
              />
              <input
                id="name"
                ...
                className={`w-full px-4 py-3 pl-10 bg-zinc-900/50 rounded-xl text-white placeholder-zinc-700 outline-none transition-all ${
                  errors.name
                    ? "border border-red-500/50 focus:ring-1 focus:ring-red-500/50 text-red-100"
                    : "border border-zinc-800 focus:border-white/20 focus:ring-1 focus:ring-white/20 hover:border-zinc-700"
                }`}
              />
              {errors.name && (
                <AlertCircle
                  className="absolute right-4 top-3.5 h-4 w-4 text-red-500"
                  aria-hidden="true"
                />
              )}
            </motion.div>
            {errors.name && (
              <p
                ...
                role="alert"
              >
                {t("minimaldark.name_req", "Required field")}
              </p>
            )}
          </div>
          ...
          <div className="group space-y-2">
            <label
              htmlFor="email"
              className={`text-[11px] font-bold uppercase tracking-widest ml-1 transition-colors ${errors.email ? "text-red-500" : "text-zinc-500 group-focus-within:text-white"}`}
            >
            ...
            >
              <Mail
                className={`absolute left-4 top-3.5 h-4 w-4 transition-colors ${errors.email ? "text-red-500" : "text-zinc-500 group-focus-within:text-white"}`}
                aria-hidden="true"
              />
              <input
                id="email"
            ...
            {errors.email && (
              <p
                ...
                role="alert"
              >
                {t("minimaldark.email_req", "Invalid email")}
              </p>
            )}
          </div>
          ...
          <div className="group space-y-2">
            <label
              htmlFor="password"
              className={`text-[11px] font-bold uppercase tracking-widest ml-1 transition-colors ${errors.password ? "text-red-500" : "text-zinc-500 group-focus-within:text-white"}`}
            >
            ...
            >
              <Lock
                className={`absolute left-4 top-3.5 h-4 w-4 transition-colors ${errors.password ? "text-red-500" : "text-zinc-500 group-focus-within:text-white"}`}
                aria-hidden="true"
              />
              <input
                id="password"
            ...
            <p
              id="pw-hint"
              className={`text-[10px] text-right ${errors.password ? "text-red-500" : "text-zinc-600"}`}
              role={errors.password ? "alert" : "status"}
            >
          ...
          >
            ...
            {loading
              ? t("minimaldark.creating", "Creating Account...")
              : t("minimaldark.create", "Create Account")}
            {!loading && (
              <ArrowRight
          ...
          <div className="relative py-4" aria-hidden="true">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="px-3 bg-[#09090b] text-zinc-500 uppercase tracking-wider">
                {t("minimaldark.or", "Or continue with")}
              </span>
            </div>
          </div>
          ...
          <button
            type="button"
            aria-label={t("minimaldark.github_aria", "Sign in with GitHub")}
            className="w-full bg-zinc-900 border border-zinc-800 text-white py-3.5 rounded-xl font-medium hover:bg-zinc-800 hover:border-zinc-700 transition-all flex items-center justify-center gap-2 group focus:ring-2 focus:ring-zinc-600 outline-none"
          >
          ...
          <p className="text-xs text-zinc-600 text-center leading-relaxed mt-4">
            {t(
              "minimaldark.agree_p1",
              "By creating an account, you agree to our",
            )}{" "}
            <a
              href="#"
              className="text-zinc-500 hover:text-white underline transition-colors focus:text-white outline-none"
            >
